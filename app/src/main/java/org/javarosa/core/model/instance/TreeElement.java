 ...
 */
 ...
 public class TreeElement implements Externalizable {
	...
	public void populate(TreeElement incoming, FormDef f) {
		...
		//incoming contiene il treeElement con i valori immessi dall'utente ma non tipizzati..
		
		if (this.isLeaf()) {
			// chiamata ricorsiva dalla riga 835 child.populate, dove viene passato solo un nodo singolo
			...
			if (value == null) {
				...
				IAnswerData typedVal = RestoreUtils.xfFact.parseData(textVal, this.dataType, this.getRef(), f);  //Effettuo la tipizzazione del dato e recupero il valore.
				this.setValue(typedVal);// setto il valore tipizzato
			}
		} else {
			Vector names = new Vector();
			for (int i = 0; i < this.getNumChildren(); i++) {
				TreeElement child = this.getChildAt(i); //Recupero i nodi tipizzati dallo schema privi del valore immesso dall'utente.
				if (!names.contains(child.getName())) {
					names.addElement(child.getName()); //inserisco il nome della domanda in un array di nomi
				}
			}
			...
			for (int i = 0; i < this.getNumChildren(); i++) {
				...
				if (child.repeatable && child.getMult() != TreeReference.INDEX_TEMPLATE) {  // --> i template dei roster sono identificati dalla proprieta' MULTIPLICITY==-2
					this.removeChildAt(i);  //Rimuovo tutte le compilazioni dei roster...lasciando solo la "definizione del roster"
					i--;
				}
			}
			...
			for (int i = 0; i < this.getNumChildren(); i++) {   //Controllo che l'array dei nomi delle domande e il treeElement siano allineati
				TreeElement child = this.getChildAt(i);
			...
			for (int i = 0; i < this.getNumChildren(); i++) {
				TreeElement child = this.getChildAt(i); //nodo tipizzato ma senza il valore immesso dall'utente
				Vector newChildren = incoming.getChildrenWithName(child.getName());  //(savedRoot) nodo non tipizzato ma con il il valore salvato dell'utente

				if (child.repeatable) { //Controllo se il nodo e' un roster...
				    for (int k = 0; k < newChildren.size(); k++) {  //Ciclo tutte le k compilazioni effettuate dall'utente...
				        TreeElement newChild = child.deepCopy(true);
				        newChild.setMult(k);
				        this.children.insertElementAt(newChild, i + k + 1); //Inserisco l'elemento...
				        newChild.populate((TreeElement)newChildren.elementAt(k), f);  //Lo popolo con i valori immessi dall'utente...
				    }
				    i += newChildren.size();
				} else {
